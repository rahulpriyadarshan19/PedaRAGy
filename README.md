...
graph TB
    ...
    subgraph "Processing Layer"
        J[PDF Extractor] --> K[Text Chunking]
        K --> L[Embedding Model]
        L --> M[Vector Store]
    end
    
    subgraph "Storage Layer"
        N[Pinecone Vector DB] --> O[Document Index]
        N --> P[Cache Index]
    end
    
    subgraph "AI Layer"
        Q[Ollama LLM] --> R[Prompt Templates]
        R --> S[Response Generation]
    end
    
    subgraph "Caching Layer"
        T[Semantic Cache] --> U[Similarity Matching]
        U --> V[Response Retrieval]
    end
